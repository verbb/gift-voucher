<?php
namespace GiftVoucher\License;

use function Craft\craft;
use Craft\DateTimeHelper;
use Craft\EtException;
use Craft\LogLevel;
use Craft\GiftVoucher_LicenseModel;
use Craft\GiftVoucherPlugin;

class GiftVoucher_License
{
    ...
    public function __construct($endpoint, $product, $productVersion, $licenseKey = '')
    {
        ...
        $this->_model = new GiftVoucher_LicenseModel($attributes);
        ...
        try {
            ...
            if (!craft()->cache->get($this->etConnectFailureKey)) {
                ...
                if ($response->isSuccessful()) {
                    ...
                    $etModel = craft()->giftVoucher_license->decodeEtModel($response->getBody());

                    if ($etModel) {
                        return $etModel;
                    } else {
                        GiftVoucherPlugin::log('Error in calling ' . $this->_endpoint . ' Response: ' . $response->getBody(), LogLevel::Warning);
            ...
            GiftVoucherPlugin::log('Error in ' . __METHOD__ . '. Message: ' . $e->getMessage(), LogLevel::Error);
